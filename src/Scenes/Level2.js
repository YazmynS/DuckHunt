...
//Text Setup
        ...
        document.getElementById('description').innerHTML = '<h2>Level 2</h2><br>A: left // D: right // Space: fire // S: Next Level'
...
//duck Movement
    moveTree()
    {
        ...
        if (my.sprite.tree.x > 750){ 
            while(my.sprite.tree.x > 0)
            {
                my.sprite.tree.x -= this.duckSpeed;
            }
            my.sprite.tree.x += this.duckSpeed;
        }
        if(my.sprite.tree.x == 0)
            {
                my.sprite.tree.x += this.duckSpeed;
            }
        else{
            my.sprite.tree.x += this.duckSpeed; // default move duck forward  
        }
        my.sprite.tree.x = my.sprite.tree.x;
    }
    ...
    moveDuck1()
    {
        ...
        if (my.sprite.duck1.x > 750){ 
            while(my.sprite.duck1.x > 0)
            {
                my.sprite.duck1.x -= this.duckSpeed;
            }
            my.sprite.duck1.x += this.duckSpeed;
        }
        if(my.sprite.duck1.x == 0)
            {
                my.sprite.duck1.x += this.duckSpeed;
            }
        else{
            my.sprite.duck1.x += this.duckSpeed; // default move duck forward  
        }
        my.sprite.dStand1.x = my.sprite.duck1.x;
    }
    moveDuck2(){
        let my = this.my;
        
        if (my.sprite.duck2.x > 740){
            my.duck2Forward = true;
        }else if (my.sprite.duck2.x < 0){
            my.duck2Forward = false;
        }
       
        if (!my.duck2Forward){
            my.sprite.duck2.x += this.duckSpeed;
        }else{
            my.sprite.duck2.x -= this.duckSpeed;
        }
        my.sprite.dStand2.x = my.sprite.duck2.x;
    }

    update() {
        let my = this.my;
        //tree shoots after certain time period passes
        ...
        for (let bullet of my.sprite.bullet) {
            ...
            if (this.collides(my.sprite.duck1, bullet)) {
                ...
                my.sprite.dStand1.visible = false;
                ...
                this.puff.on(Phaser.Animations.Events.ANIMATION_COMPLETE, () => {
                    my.sprite.duck1.visible = true;
                    my.sprite.dStand1.visible = true;
                    my.sprite.duck1.x = Math.random()*config.width - 50;
                }, this);
            }
                ...
                my.sprite.dStand2.visible = false;
                ...
                // Have new duck appear after end of animation
                this.puff.on(Phaser.Animations.Events.ANIMATION_COMPLETE, () => {
                    my.sprite.duck2.visible = true;
                    my.sprite.dStand2.visible = true;
                    my.sprite.duck2.x = Math.random()*config.width - 10;
                }, this);
            }
        }
...
//Level 1 Tasks
    //Ducks
        //When hit ducks retreat into the water and resurface in a different place 
        //Create an array of ducks so there's more than two (Stretch Goal)
    //Environment (Need Help with all of these)
        //Timer must count down from 90
        //Score Text must match game text (Stretch Goal)
        //Title Text must match game text (Stretch Goal)
        //Have Targets move at varied speeds (Stretch Goal)
    // Tree
        //Tree shoots 3 branches when shot but shots 1 after a little while regardless (Stretch Goal)
        //Trees should move like the ducks do
//Level 2 Tasks
    //Environment 
        //Increase target speeds
        //branch speed should also increase
    //Targets
        //Change movement some ducks move up and down and others move across the screen like in level 1
        //create more trees
